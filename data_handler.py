...
# Load text data
def load(fea_types, fea_path, dataset_file, vocab, include_caption='none', separate_caption=False,
         max_history_length=-1, merge_source=False, undisclosed_only=False):
    dialog_data = json.load(open(dataset_file, 'r'))
    dialog_list = []
    vid_set = set()
    qa_id = 0
    for dialog in dialog_data['dialogs']:
        ...
        for n in it:
            ...
            if (
                    include_caption == 'caption' or include_caption == 'summary' or include_caption == 'caption,summary') and separate_caption:
    ...
    for j in six.moves.range(n_seqs):
        ...
        if j == 0:
            ...
            x_batch = [np.ones((x_len[i], n_seqs, fea[i].shape[-1]), dtype=np.float32)
                       if len(fea[i].shape) == 2 else np.zeros((x_len[i], n_seqs, fea[i].shape[-2], fea[i].shape[-1]),
                                                               dtype=np.float32)
